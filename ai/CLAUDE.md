...
- **생성 실패 신호 공용 가드 본 구현 (F4 일원화)**: 질문 풀·꼬리질문·피드백 3개 consumer 의 실패
  ...
  1.5s 하드 타임아웃과 `llm_pro_timeout_sec` 60s/`llm_flash_timeout_sec` 10s 요청 타임아웃은
  이전 리팩터에서 도입되어 유지된다 — Pro 는 30s 로 시작했으나 운영 성공 호출이 p90 31.9초,
  최대 56초(=타임아웃 30초 + 내부 재시도 26초)로 찍히고 문서 분석이 3회 모두 30초를 넘겨
  영구 실패해 60s 로 올렸다.)
