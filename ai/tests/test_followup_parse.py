...
def test_extract_question_only_helper():
    ...
    assert (
        extract_question_span(
            "<intent>NORMAL</intent><question>안녕</question><meta>{}"
        )
        == "안녕"
    )
...
def test_streaming_keeps_lt_in_body():
    ...
    assert (
        extract_question_span(
            "<intent>NORMAL</intent><question>제네릭 List<T> 를 설명하세요"
        )
        == "제네릭 List<T> 를 설명하세요"
    )
...
def test_streaming_strips_partial_closing_or_meta_tag():
    ...
    assert (
        extract_question_span("<intent>NORMAL</intent><question>질문입니다?</quest")
        == "질문입니다?"
    )
    assert (
        extract_question_span("<intent>NORMAL</intent><question>끝났나요?<met")
        == "끝났나요?"
    )
