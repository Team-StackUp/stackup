...
def test_auto_falls_back_to_mock_without_key():
    s = _settings(tts_provider="auto", gemini_api_key="")
    assert isinstance(build_tts_provider(s), MockTtsProvider)


def test_explicit_mock():
    s = _settings(tts_provider="mock", gemini_api_key="g-test")
    assert isinstance(build_tts_provider(s), MockTtsProvider)


def test_auto_picks_gemini_without_gateway_key():
    # 게이트웨이 키가 없으면 직접 Gemini 키로.
    s = _settings(tts_provider="auto", gemini_api_key="g-test")
