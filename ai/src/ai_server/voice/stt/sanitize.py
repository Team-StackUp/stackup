...
# CTA(행동 유도) 동사 — 구독/좋아요 류 환각의 결정적 신호. 이게 있어야만 제거한다
# (정상 답변의 "구독/좋아요/알림 설정 기능"을 오삭제하지 않기 위함).
_CTA = (
    r"(?:부탁\s*(?:드립니다|드려요|해요|합니다)|"
    r"눌러\s*주세요|꾹\s*눌러\s*주세요|클릭\s*(?:해\s*)?주세요)"
)
...
_HALLUCINATION_PATTERNS = [
    ...
    for p in (
        ...
        # 시청/시청 인사 (봐주셔서·끝까지 포함)
        r"(?:끝까지\s*)?(?:봐|시청\s*해)\s*주(?:셔서|시고)\s*감사(?:합니다|해요|드립니다|드려요)?",
        r"오늘도\s*시청해\s*주셔서[^.\n]{0,20}",
        r"영상\s*을?\s*시청해\s*주셔서[^.\n]{0,20}",
        r"오늘\s*영상은?\s*여기까지(?:입니다|이에요|예요)?",
        r"이\s*영상이?\s*(?:도움이?\s*)?(?:되셨다면|되었다면|유익하셨다면)[^.\n]{0,20}",
        # 구독/좋아요/알림설정 유도 — CTA 동사가 바로 따라올 때만 제거.
        rf"(?:구독|좋아요|알림\s*설정)"
        rf"(?:\s*(?:과|와|,|및|랑|\s)\s*(?:구독|좋아요|알림\s*설정)){{0,2}}"
        rf"\s*(?:버튼|영상)?\s*{_CTA}",
        # 다음 영상/편 인사
